...
{%- for r in site.data.db_stats.by_state %}
    <tr>
      ...
      <td class="num">{% include num.html n=r.batches %}</td>
      <td class="num">{% include num.html n=r.images %}</td>
      <td class="num">{% include num.html n=r.primary_cutouts %}</td>
      <td class="num">{% include num.html n=r.cutouts %}</td>
      <td class="num">{% include num.html n=r.detections %}</td>
    </tr>
{%- endfor %}
    <tr class="total-row">
...
{%- for r in site.data.db_stats.by_size %}
    <tr>
      <td>{% if r.size_class == "unbinned" %}Unbinned{% else %}{{ r.size_class }} cm²{% endif %}</td>
      <td class="num">{% if r.primary_cutouts == 0 %}–{% else %}{% include num.html n=r.primary_cutouts %}{% endif %}</td>
      <td class="num">{% include num.html n=r.cutouts %}</td>
      <td class="num">{% include num.html n=r.detections %}</td>
    </tr>
{%- endfor %}
...
{%- for sp in site.data.db_stats.species %}
    <tr>
      ...
      <td>{{ sp.group }}</td>
      <td class="num">{% if sp.primary_cutouts == 0 %}–{% else %}{% include num.html n=sp.primary_cutouts %}{% endif %}</td>
      <td class="num">{% if sp.cutouts == 0 %}–{% else %}{% include num.html n=sp.cutouts %}{% endif %}</td>
      <td class="num">{% include num.html n=sp.detections %}</td>
    </tr>
{%- endfor %}
    <tr class="total-row">
      <td><strong>Total</strong></td>
...
{%- for v in sp.primary_by_size %}
      <td class="num">{% if v == 0 %}–{% else %}{% include num.html n=v %}{% endif %}</td>
{%- endfor %}
      <td class="num"><strong>{% include num.html n=sp.primary_cutouts %}</strong></td>
    </tr>
{%- endif %}
{%- endfor %}
    <tr class="total-row">
